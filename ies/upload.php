...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_FILES['csv_file']) && $_FILES['csv_file']['error'] === UPLOAD_ERR_OK) {
        ...
        // Define the expected header (update to match your CSV)
        ...
        while (($row = fgetcsv($file)) !== false) {
            ...
            $data = [
                ...
                'state' => $row[3],
                'city' => $row[4],
                'program' => $row[5],   // Program
                'course' => $row[6],
                'submitted_to_api' => 0 // Default is 0 (not submitted_to_api)
            ];

            // Prepare the SQL statement
            $stmt = $conn->prepare("INSERT INTO ies (name, email, mobile, state, city, program, course, submitted_to_api) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
            ...
            $stmt->bind_param('ssssssss', $data['full_name'], $data['email'], $data['phone_number'], $data['state'], $data['city'], $data['program'], $data['course'], $data['submitted_to_api']);
