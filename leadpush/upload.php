...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_FILES['csv_file']) && $_FILES['csv_file']['error'] === UPLOAD_ERR_OK) {
        ...
        while (($row = fgetcsv($file)) !== false) {
            ...
            $stmt = $conn->prepare("INSERT INTO manipal (full_name, email, mobile, state, city, program, course, submitted) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
            ...
            $stmt->bind_param('ssssssss', $data['full_name'], $data['email'], $data['mobile'], $data['state'], $data['city'], $data['program'], $data['course'], $data['submitted']);
